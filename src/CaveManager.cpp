...
bool CaveManager::isAir(int bX, int bY)
{
	for (int i = 0; i < MAX_CAVE_LENGTH / CAVE_CREATE_X; i++)
	{
		...
		caveRNoise->setSeed(caveSeed * 1.78);
		int perlinR = caveRNoise->getPerlinVal(bX - caveBX + perlinY, CAVE_MAX_R, 0);
		int lastPerlinR = caveRNoise->getPerlinVal(bX - caveBX + lastPerlinY, CAVE_MAX_R, 0);
		if (perlinY < lastPerlinY)
		{
			if (bY >= perlinY - perlinR && bY <= lastPerlinY + lastPerlinR)
			{
				return true;
			}
		}
		...
		{
			if (bY >= lastPerlinY - lastPerlinR && bY <= perlinY + perlinR)
			{
				return true;
			}
		}
		for (int j = 1; j < CAVE_MAX_R; j++)
		{
			int cPerlinY1 = caveYNoise->getPerlinVal(bX - caveBX - j, caveMaxY - caveMinY, caveMinY);
			int cPerlinY2 = caveYNoise->getPerlinVal(bX - caveBX - j - 1, caveMaxY - caveMinY, caveMinY);
			int cPerlinR1 = caveRNoise->getPerlinVal(bX - caveBX - j + cPerlinY1, );
			int cPerlinR2 = caveRNoise->getPerlinVal(bX - caveBX - j + cPerlinY2, )
			if (cPerlinY1 < cPerlinY2)
			{
				if (bY >= perlinY - perlinR && bY <= lastPerlinY + lastPerlinR)
				{
