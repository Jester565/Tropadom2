	...
	:boxCore(boxCore), boxBody(nullptr), vol(1), pan(0), idVol(1)
{
	...
	lightSource = new lighting::CircleLightSource(boxCore->getLightLayer(), 600);
	Core::GetSoundManager()->addIDVol(1.0);
	monsterSong = new AllegroExt::Sound::Music(Core::GetSoundManager(), "monster.ogg");
	grassStep = new AllegroExt::Sound::SoundEffect(Core::GetSoundManager(), "grass1.wav", 0);
...
void GameTest::draw()
{
	if (AllegroExt::Input::InputManager::getClicked())
	{
		...
		boxBody = new PhysicsImgLight(boxCore->getLightLayer(), boxCore->getWorldManager(), BODY_DYNAM, AllegroExt::Input::InputManager::mouseX, 10);
		...
		((PhysicsImgLight*)boxBody)->initPoly("triangle.png", 4, 4);
	...
	if (AllegroExt::Input::InputManager::keyPressed('m'))
	{
		...
		Core::GetSoundManager()->setIDVol(0, idVol);
